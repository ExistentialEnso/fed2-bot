...
async function setProtectiveStockpiles(connection) {
    ...
    for(let c of commods) {
        ...
        if(match) {
            ...
            if(max === 20000 && stock < 5000) {
                await connection.send("set stockpile max " + 100 + " " + commod)
            } else if (min === 100 && max === 20000 && stock > 10000) {
                await connection.send("set stockpile max " + 10000 + " " + commod)
            }
        }
    }
}
...
module.exports = {
    ...
    setProtectiveStockpiles: setProtectiveStockpiles,
